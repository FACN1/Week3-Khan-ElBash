...
QUnit.test("test the function converts the response to an array of length 3", function (assert) {
  ...
  var result = Render.prepareGIFUrls(null, response);
...
QUnit.test("test the function logs the error if passed an error", function (assert) {
  var result = Render.prepareGIFUrls(404, null);
  assert.equal(result, undefined, "function should not return anything if receives an error");
});
